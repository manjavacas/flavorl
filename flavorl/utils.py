...
def extract_times_and_clean(text: str):
    ...
    for m in matches:
        ...
        if label == 'prep' and 'prep' not in seen:
            prep_val = val; seen.add('prep'); end_cut = max(end_cut, m.end())
        elif label == 'cook' and 'cook' not in seen:
            cook_val = val; seen.add('cook'); end_cut = max(end_cut, m.end())
        elif label == 'readyin' and 'readyin' not in seen:
            ready_val = val; seen.add('readyin'); end_cut = max(end_cut, m.end())
    ...
    if len(seen) < 3:
        ...
        if prep_val is None:
            ...
            if m: prep_val = m.group(1).strip()
        if cook_val is None:
            m = re.search(r'\bCook\b\s*[:\-]?\s*([^\n\r]+)', head, flags=re.I)
            if m: cook_val = m.group(1).strip()
        if ready_val is None:
            m = re.search(r'\bReady\s*In\b\s*[:\-]?\s*([^\n\r]+)', head, flags=re.I)
            if m: ready_val = m.group(1).strip()
