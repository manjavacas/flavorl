...
from utils import extract_times_and_clean, _clean_and_unpack

# 1) RUTAS 
DATA_DIR = Path("data/MealRecPlus/MealRec+/MealRec+H")
...
# Comprobaciones
required_paths = [
    course_csv_fp, user_course_csv_fp, user2index_fp, course2index_fp,
    user_course_txt_fp, course_category_fp, meal_course_fp,
    course_fsa_fp, course_who_fp, meal_fsa_fp, meal_who_fp, user_fsa_fp, user_who_fp
]
...
df_courses[['cooking_directions','prep_min','cook_min','ready_min']] = (
    df_courses['cooking_directions'].apply(_clean_and_unpack)
)
...
course2index_fp = "data/MealRecPlus/MealRec+/MealRec+H/meta_data/course2index.txt"
course_fsa_fp   = "data/MealRecPlus/MealRec+/MealRec+H/healthiness/course_fsa.txt"
course_who_fp   = "data/MealRecPlus/MealRec+/MealRec+H/healthiness/course_who.txt"

# --- 1) course_id -> course_index ---
df_c2i = pd.read_csv(course2index_fp, sep="\t", header=None, names=["course_id", "course_index"])
...
s_fsa.index.name = "course_index"; s_fsa.name = "course_fsa"
s_who.index.name = "course_index"; s_who.name = "course_who"
...
print("Missing FSA:", df_courses["course_fsa"].isna().sum(), "Missing WHO:", df_courses["course_who"].isna().sum())
